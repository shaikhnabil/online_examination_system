...
if (isset($_POST['page'])) {
			...
			if ($total_row == 0) {
				...
				foreach ($result as $row) {
					if ($row['student_email_verified'] == 'No') {
						...
						<p>This is a verification Email for your ' . $row["student_email_id"] . ' email address, and without email verification you can not take part in online exam, So please click the link to verify your Email address  <a href="' . $object->base_url . 'verify_email.php?type=student&code=' . $verification_code . '" target="_blank"><b>link</b></a>.</p>
				...
				foreach ($result as $row) {
					if ($row['student_email_verified'] == 'Yes') {
						if ($row["student_status"] == 'Enable') {
							...
							$body = '
							<p>Hello ' . $row["student_name"] . '.</p>
							<p>For login into this Online Examiantion System by visiting <a href="' . $object->base_url . 'login.php" target="_blank"><b>' . $object->base_url . 'login.php</a></b> this link. Below you can find password details.</a></p>
	...
	if($_POST['page'] == 'change_password')
		{
			if($_POST['action'] == 'change_password')
			{
				...
				$error = 'Password changed successfully.Now login again with new password';
				$output = array(
					'error'		=>	$error
				);
	...
	if ($_POST['page'] == 'exam') {
		...
		if ($_POST["action"] == 'fetch_timetable') {
			...
			WHERE subject_wise_exam.exam_id = '" . $_POST["exam_id"] . "'
			";
	...
	if($_POST['page'] == 'view_subject_exam')
	{
		if($_POST['action'] == 'load_question')
		{
			...
			{
				$output .= '
				<div class="card" style="background-color:#f3e9ff;">
				...
				';
				$object->query = "
				SELECT exam_subject_question_id FROM exam_subject_question 
				WHERE exam_subject_question_id < '".$row['exam_subject_question_id']."' 
				...
				$output .= '
				  	<div align="center">
          				...
          				<input type="submit" name="submit" id="submit_button" class="btn btn-primary btn-lg submit" value="Submit" />
				  	</div>
				  	</div></div>';
			}
		...
		if ($_POST['action'] == 'question_navigation') 
		{
			...
			$output = '
			<div class="card">
				<div class="card-header" style="background-color:#8548d2;color:#f3e9ff"><b>Question Navigation</b></div>
				<div class="card-body">
					<div class="row">
			';
			$count = 1;
		...
		//answer of question
		if ($_POST['action'] == 'answer' || $_POST['action'] == 'submit')
		 {
