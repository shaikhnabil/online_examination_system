...
error_reporting(0);
//check db_info file is available or not...
if (file_exists('db_info.inc')) {
	include('db_info.inc'); //add file
	//try to connect
	try {
	...
	$message = 'Set Mysql Database Configuration Details';
} //if database and  table are not available fill the form
...
if (isset($_POST["submit"])) {
	...
	<?php 
		...
		$edb_user_name = "' . $database_username . '";
		$edb_password = "' . $database_password . '";
		$edb_host = "' . $database_host . '";
		$ebase_url = "' . $base_url . '";
	?>';

	if (file_put_contents('db_info.inc', $string)) {
...
<head>
	...
	<title>Online Examination Setup</title>
