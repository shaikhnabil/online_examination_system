...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('user_name', 'user_contact_no', 'user_email', 'user_qualification', 'user_type', 'user_status', 'user_created_on');
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'OR user_qualification LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = $row["user_qualification"];
			...
			if($row["user_status"] == 'Enable')
			{
				...
				$status = '<button type="button" name="status_button" class="btn btn-primary btn-sm status_button" data-id="'.$row["user_id"].'" data-status="'.$row["user_status"].'">Enable</button>';
			}
			else
			{
				...
				$status = '<button type="button" name="status_button" class="btn btn-danger btn-sm status_button" data-id="'.$row["user_id"].'" data-status="'.$row["user_status"].'">Disable</button>';
			}
			$sub_array[] = $status;
			$sub_array[] = '
			<div>
			...
			&nbsp;
			<button type="button" name="delete_button" class="btn btn-danger btn-circle btn-sm delete_button" data-id="'.$row["user_id"].'"><i class="fas fa-trash-alt"></i></button>
			</div>';
			$data[] = $sub_array;
		}
...
//insert data
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			...
			(user_name, user_contact_no, user_email, user_password,user_qualification, user_profile, user_type, user_status, user_created_on) 
			VALUES (:user_name, :user_contact_no, :user_email, :user_password, :user_qualification, :user_profile, :user_type, :user_status, :user_created_on)
			";
...
//select single data
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($result as $row)
		{
			...
			$data['user_qualification'] = $row['user_qualification'];
