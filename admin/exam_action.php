...
if (isset($_POST["action"])) {
	if ($_POST["action"] == 'fetch') {
		$order_column = array('exam.exam_title', 'exam_class.class_name', 'exam.exam_duration', 'exam.exam_result_datetime', 'exam.exam_status');
		...
		ON exam_class.class_id = exam.exam_class_id 
		";
		...
		if (isset($_POST["search"]["value"])) {
			...
			$search_query .= 'OR exam.exam_title LIKE "%' . $_POST["search"]["value"] . '%" ';
			$search_query .= 'OR exam.exam_duration LIKE "%' . $_POST["search"]["value"] . '%" ';
			$search_query .= 'OR exam.exam_result_datetime LIKE "%' . $_POST["search"]["value"] . '%" ';
			$search_query .= 'OR exam.exam_status LIKE "%' . $_POST["search"]["value"] . '%" ';
		}

		if (isset($_POST["order"])) {
			$order_query = 'ORDER BY ' . $order_column[$_POST['order']['0']['column']] . ' ' . $_POST['order']['0']['dir'] . ' ';
		} else {
			$order_query = 'ORDER BY exam.exam_id DESC ';
		}
		...
		foreach ($result as $row) {
			...
			$object->query = "SELECT * FROM subject_wise_exam 
				...
				else 
				{
					...
					else 
					{
						if ($row['exam_status'] == 'Pending') 
						{
							...
							<button type="button" name="delete_button" class="btn btn-danger btn-circle btn-sm delete_button" data-id="' . $row["exam_id"] . '"><i class="fas fa-trash"></i></button>
							</div>
							';
						}
	...
	//get data for publishing result
	if ($_POST["action"] == 'fetch_result_publish_data') {
		$object->query = "SELECT * FROM exam 
		WHERE exam_id = '" . $_POST["exam_id"] . "'
		";
